...
State as of `e4bcbaa` on `main`, deployed to <https://finto.zepto.page>.
...
**Screenshots.** `puppeteer-core` driving the system Chrome at 390×844 @2x is
what produced `docs/design/mobile/*.png`. There is no capture script in the
repo — write one if you need it, it is ~40 lines.
...
## 3. What is open — ordered by visible change per unit of work

Nothing below is started. The first three are the ones that change how the app
*feels*, and they are small.

### 3.1 Stop painting spending red
...
### 3.2 Labels to sentence-case sans
...
### 3.3 Human dates, slimmer bands, drop the repeated currency
...
### 3.4 Fewer borders — separate with surface, not line
...
### 3.6 Expose the classifier; lead the picker with its guess
...
both. Net worth belongs on Accounts. **This is a product decision — ask the
...
### 3.11 Controls are still mouse-shaped
...
became multithreaded during startup"* and ~154 of 332 tests error at fixture
setup, looking exactly like a missing PostgreSQL. It is not.
`LC_ALL=en_US.UTF-8 .venv/bin/pytest -q` → 332 passed in ~7.5 min. Stream it or
background it; piping to `tail` hides all progress for eight minutes.
...
LC_ALL=en_US.UTF-8 .venv/bin/pytest -q           # 332 passed
```
...
- The design assessment behind this work, including the competitive survey and
  the reasoning for §3.1–3.8: ask the owner for the artifact link.
