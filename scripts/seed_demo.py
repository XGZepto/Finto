"""Seed a local database with invented transactions, for UI work.
...
from fin.models import (Account, Card, FileFormat, Institution, Money, StatementFile,
                        Txn, TxnKind, FxRate)
...
def main() -> None:
    ...
    def add(account_id, d, amount, currency, desc, merchant, category, sub,
        ...
        txns.append(Txn(
            ...
            booked=Money(amount=int(round(amount * 100)), currency=currency),
    ...
    while d < today:
        ...
        for name in SUBSCRIPTIONS:
            ...
            if ccy == "USD" and acct == "amex_hk_main":
                native = Money(amount=int(round(-amt * 100)), currency="USD")
                fx = Decimal("7.81")
                booked = -amt * 7.81
                bccy = "HKD"
            add(acct, d + timedelta(days=(hash(name) % 26) + 1), booked, bccy,
